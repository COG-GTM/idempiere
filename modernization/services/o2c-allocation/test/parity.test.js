...
// accounting exactly (balanced Fact_Acct, correct realized FX). The realized FX
// balancing line must always post so multi-currency allocations balance, even
// when the ALLOC_BUG demo gate is armed.
...
test('EUR allocation 601 books a 25.00 realized FX loss and balances', async () => {
  ...
  assert.equal(debit, credit);         // DR == CR
...
test('FX FIX: EUR allocation 601 posts balanced (DR == CR) even with ALLOC_BUG gate armed', async () => {
  const alloc = await loadAllocation(601);
  const { facts, balanced, debit, credit } = await buildFacts(alloc, { buggy: true });
  assert.ok(balanced, 'realized FX line must post even with the gate armed');
  assert.equal(debit, credit);         // DR == CR
  assert.equal(debit, 550);
  assert.equal(credit, 550);
  assert.equal(byType(facts).RealizedLoss, 25); // balancing line is present
...
test('USD allocation 600 posts balanced regardless of the gate (single-currency)', async () => {
  ...
  assert.equal(res.posted, true); // no FX => balances
  assert.equal(res.debit, res.credit);
});
