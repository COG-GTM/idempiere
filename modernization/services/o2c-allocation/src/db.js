...
function createPool() {
  if (shouldUseEmbeddedDb()) return embeddedPool;
  const config = process.env.DATABASE_URL ? buildPgPoolConfig() : buildDiscretePoolConfig();
...
module.exports = {
  ...
  _testOnly: { isLocalHostname, buildPgPoolConfig, buildDiscretePoolConfig, createPool },
};
