...
// GL balance re-derivation. The Oracle-era reconciliation had a covering index
// on Fact_Acct (AD_Table_ID, Record_ID) INCLUDE (AmtAcctDr, AmtAcctCr) so this
// was an index-only scan. The initial Oracle->PG migration dropped that index,
// causing the query to fall back to a sequential scan and spiking
// o2c.allocation.posting.duration. Fixed: the covering index is now restored in
// schema.sql (idx_fact_acct_recon).
